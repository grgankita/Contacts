...
const BinarySearchTree_js_1 = __importDefault(require("./BinarySearchTree.js"));
const node_js_1 = __importDefault(require("./node.js"));
const contact_js_1 = __importDefault(require("./contact.js"));
class AVLNode extends node_js_1.default {
    constructor(value) {
        super(value);
        this.height = 1; // New nodes initially have a height of 1
    }
}
class AVLTree extends BinarySearchTree_js_1.default {
    constructor() {
        super();
    ...
    insert(contact) {
        ...
        if (this.root === null) {
            ...
            console.log(` Added root contact: ${contact.name}. Tree size: ${this.size}`);
            return true;
        }
        ...
        while (!inserted) {
            ...
            if (compareResult === 0) {
                ...
                if (currentNode.left === null) {
                    ...
                    inserted = true;
                    console.log(` AVLTree ${this.instanceId}: Added ${contact.name} to left. New size: ${this.size}`);
                }
                else {
                ...
                if (currentNode.right === null) {
                    ...
                    this.size++; // <--- Increment size HERE when linked to the tree
                    inserted = true;
                    console.log(` AVLTree ${this.instanceId}: Added ${contact.name} to right. New size: ${this.size}`);
                }
                else {
    ...
    _inOrderCollect(node, contacts) {
        if (node) {
            ...
            console.log("Collecting node:", node.value ? node.value.name : "N/A", node.value instanceof contact_js_1.default);
    ...
    getContactsNewlyAddedFirst() {
        ...
        console.log("Contacts before sort by dateAdded:", contacts.map((c) => ({ name: c.name, dateAdded: c.dateAdded })));
        contacts.sort((a, b) => {
        ...
        console.log(" Contacts after sort:", contacts.map((c) => ({ name: c.name, dateAdded: c.dateAdded })));
        return contacts;
    }
    getContactsMostRecentActivityFirst() {
        const contacts = this._inOrderCollect(this.root, []);
        console.log(" Contacts before sort by lastActivity:", contacts.map((c) => ({ name: c.name, lastActivity: c.lastActivity })));
        contacts.sort((a, b) => {
            const dateA = a.lastActivity instanceof Date ? a.lastActivity.getTime() : 0;
            const dateB = b.lastActivity instanceof Date ? b.lastActivity.getTime() : 0;
            return dateB - dateA; // Descending order (most recent first)
        });
        console.log(" Contacts after sort:", contacts.map((c) => ({ name: c.name, lastActivity: c.lastActivity })));
        return contacts;
    }
}
